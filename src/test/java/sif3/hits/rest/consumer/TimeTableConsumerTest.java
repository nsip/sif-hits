...
public class TimeTableConsumerTest extends BaseTest<TimeTableType, TimeTableCollectionType> {
    ...
    public void setup() {
        ...
        timeTableTester = new ConsumerTest<>(TimeTableType.class, "TimeTable", TimeTableCollectionType.class, "TimeTables");
    ...
    public void testUpdateDays() {
        TimeTableType initialObject = getSingleObjectFromXML(timeTableTester.getFileContents(testData.getSingleObjectXMLFilename()));
        TimeTableType updatedObject = getSingleObjectFromXML(timeTableTester.getFileContents(testData.getSingleObjectXMLFilename()));
    ...
    public void testDeleteFailsConstraint() {
        ...
        Assert.assertEquals("HTTP Status Response not 500", HttpStatus.INTERNAL_SERVER_ERROR.value(), deleteResponse.getStatus());
        Assert.assertTrue("Error message not as expected", deleteResponse.getError().getMessage().contains("Cannot delete object because it has child objects"));
    }
}
