...
public class CalendarDateConsumerTest extends BaseTest<CalendarDateType, CalendarDateCollectionType> {
    ...
    public void setup() {
        ...
        calendarDateTester = new ConsumerTest<>(CalendarDateType.class, "CalendarDate", CalendarDateCollectionType.class, "CalendarDates");
    ...
    public int getCollectionSize(CalendarDateCollectionType collection) {
        return Optional.ofNullable(collection).map(CalendarDateCollectionType::getCalendarDate).map(List::size).orElse(-1);
    }

    @Override
    public List<CalendarDateType> getCollectionList(CalendarDateCollectionType collection) {
        return Optional.ofNullable(collection).map(CalendarDateCollectionType::getCalendarDate).orElse(new ArrayList<>());
    }
    ...
    public void testUpdateSingleOtherCodes() {
        CalendarDateType initialObject = getSingleObjectFromXML(calendarDateTester.getFileContents(testData.getSingleObjectXMLFilename()));
        CalendarDateType updatedObject = getSingleObjectFromXML(calendarDateTester.getFileContents(testData.getSingleObjectXMLFilename()));
