...
public class SessionInfoConsumerTest extends BaseTest<SessionInfoType, SessionInfoCollectionType> {
	...
	public void setup() {
		...
		sessionInfoTester = new ConsumerTest<>(SessionInfoType.class, "SessionInfo", SessionInfoCollectionType.class,
				"SessionInfos");
	...
	@Override
	public void setRefId(SessionInfoType object, String refId) {
	...
	public int getCollectionSize(SessionInfoCollectionType collection) {
		return Optional.ofNullable(collection).map(SessionInfoCollectionType::getSessionInfo).map(List::size)
				.orElse(-1);
	}
