...
public class TimeTableSubjectConsumerTest extends BaseTest<TimeTableSubjectType, TimeTableSubjectCollectionType> {
    ...
    public void setup() {
        ...
        timeTableSubjectTester = new ConsumerTest<>(TimeTableSubjectType.class, "TimeTableSubject", TimeTableSubjectCollectionType.class, "TimeTableSubjects");
    ...
    public int getCollectionSize(TimeTableSubjectCollectionType collection) {
        return Optional.ofNullable(collection).map(TimeTableSubjectCollectionType::getTimeTableSubject).map(List::size).orElse(-1);
    }

    @Override
    public List<TimeTableSubjectType> getCollectionList(TimeTableSubjectCollectionType collection) {
        return Optional.ofNullable(collection).map(TimeTableSubjectCollectionType::getTimeTableSubject).orElse(new ArrayList<>());
    }
    ...
    public void testUpdateCodes() {
        TimeTableSubjectType initialObject = getSingleObjectFromXML(timeTableSubjectTester.getFileContents(testData.getSingleObjectXMLFilename()));
        TimeTableSubjectType updatedObject = getSingleObjectFromXML(timeTableSubjectTester.getFileContents(testData.getSingleObjectXMLFilename()));
