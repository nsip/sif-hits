...
public class WellbeingEventConsumerTest extends BaseTest<WellbeingEventType, WellbeingEventCollectionType> {
	...
	public void setup() {
		...
		wellbeingEventTester = new ConsumerTest<>(WellbeingEventType.class, "WellbeingEvent",
				WellbeingEventCollectionType.class, "WellbeingEvents");
	...
	@Override
	public void setRefId(WellbeingEventType object, String refId) {
		object.setRefId(refId);
	}

	@Override
	public int getCollectionSize(WellbeingEventCollectionType collection) {
		return Optional.ofNullable(collection).map(WellbeingEventCollectionType::getWellbeingEvent).map(List::size)
				.orElse(-1);
	}

	@Override
	public List<WellbeingEventType> getCollectionList(WellbeingEventCollectionType collection) {
		return Optional.ofNullable(collection).map(WellbeingEventCollectionType::getWellbeingEvent)
