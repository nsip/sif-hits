...
public class StaffPersonalConsumerTest extends BaseTest<StaffPersonalType, StaffPersonalCollectionType> {
    ...
    public void setup() {
        ...
        staffPersonalTester = new ConsumerTest<>(StaffPersonalType.class, "StaffPersonal", StaffPersonalCollectionType.class, "StaffPersonals");
    ...
    public List<StaffPersonalType> getCollectionList(StaffPersonalCollectionType collection) {
        return Optional.ofNullable(collection).map(StaffPersonalCollectionType::getStaffPersonal).orElse(new ArrayList<>());
    }

    @Override
    public int getCollectionSize(StaffPersonalCollectionType collection) {
        return Optional.ofNullable(collection).map(StaffPersonalCollectionType::getStaffPersonal).map(List::size).orElse(-1);
    }
    ...
    public void testDoubleByteName() {
        StaffPersonalType initialObject = getSingleObjectFromXML(staffPersonalTester.getFileContents(testData.getSingleObjectXMLFilename()));
        StaffPersonalType updatedObject = getSingleObjectFromXML(staffPersonalTester.getFileContents(testData.getSingleObjectXMLFilename()));
