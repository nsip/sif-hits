...
public class NAPEventStudentLinkConsumerTest extends BaseTest<NAPEventStudentLinkType, NAPEventStudentLinkCollectionType> {
    ...
    public void setup() {
        ...
        napEventStudentLinkTester = new ConsumerTest<>(NAPEventStudentLinkType.class, "NAPEventStudentLink", NAPEventStudentLinkCollectionType.class, "NAPEventStudentLinks");
    ...
    public List<NAPEventStudentLinkType> getCollectionList(NAPEventStudentLinkCollectionType collection) {
        return Optional.ofNullable(collection).map(NAPEventStudentLinkCollectionType::getNAPEventStudentLink).orElse(new ArrayList<>());
    }

    @Override
    public int getCollectionSize(NAPEventStudentLinkCollectionType collection) {
        return Optional.ofNullable(collection).map(NAPEventStudentLinkCollectionType::getNAPEventStudentLink).map(List::size).orElse(-1);
    }
