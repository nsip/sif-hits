...
public class CensusCollectionConsumerTest extends BaseTest<CensusCollectionType, CensusCollectionCollectionType> {
    ...
    public void setup() {
        ...
        censusCollectionTester = new ConsumerTest<>(CensusCollectionType.class, "CensusCollection", CensusCollectionCollectionType.class, "CensusCollections");
    ...
    public int getCollectionSize(CensusCollectionCollectionType collection) {
        return Optional.ofNullable(collection).map(CensusCollectionCollectionType::getCensusCollection).map(List::size).orElse(-1);
    }

    @Override
    public List<CensusCollectionType> getCollectionList(CensusCollectionCollectionType collection) {
        return Optional.ofNullable(collection).map(CensusCollectionCollectionType::getCensusCollection).orElse(new ArrayList<>());
    }
