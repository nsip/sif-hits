...
public class GradingAssignmentConsumerTest extends BaseTest<GradingAssignmentType, GradingAssignmentCollectionType> {
    ...
    public void setup() {
        ...
        gradingAssignmentTester = new ConsumerTest<>(GradingAssignmentType.class, "GradingAssignment", GradingAssignmentCollectionType.class, "GradingAssignments");
    ...
    public List<GradingAssignmentType> getCollectionList(GradingAssignmentCollectionType collection) {
        return Optional.ofNullable(collection).map(GradingAssignmentCollectionType::getGradingAssignment).orElse(new ArrayList<>());
    }

    @Override
    public int getCollectionSize(GradingAssignmentCollectionType collection) {
        return Optional.ofNullable(collection).map(GradingAssignmentCollectionType::getGradingAssignment).map(List::size).orElse(-1);
    }
