...
public class PurchaseOrderConsumerTest extends BaseTest<PurchaseOrderType, PurchaseOrderCollectionType> {
	...
	public void setup() {
		...
		purchaseOrderTester = new ConsumerTest<>(PurchaseOrderType.class, "PurchaseOrder",
				PurchaseOrderCollectionType.class, "PurchaseOrders");
	...
	@Override
	public void setRefId(PurchaseOrderType object, String refId) {
		object.setRefId(refId);
	}

	@Override
	public List<PurchaseOrderType> getCollectionList(PurchaseOrderCollectionType collection) {
		return Optional.ofNullable(collection).map(PurchaseOrderCollectionType::getPurchaseOrder)
				.orElse(new ArrayList<>());
	}

	@Override
	public int getCollectionSize(PurchaseOrderCollectionType collection) {
		return Optional.ofNullable(collection).map(PurchaseOrderCollectionType::getPurchaseOrder).map(List::size)
				.orElse(-1);
	}
