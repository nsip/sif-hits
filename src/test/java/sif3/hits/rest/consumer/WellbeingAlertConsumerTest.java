...
public class WellbeingAlertConsumerTest extends BaseTest<WellbeingAlertType, WellbeingAlertCollectionType> {
	...
	public void setup() {
		...
		wellbeingAlertTester = new ConsumerTest<>(WellbeingAlertType.class, "WellbeingAlert",
				WellbeingAlertCollectionType.class, "WellbeingAlerts");
	...
	@Override
	public void setRefId(WellbeingAlertType object, String refId) {
		object.setRefId(refId);
	}

	@Override
	public int getCollectionSize(WellbeingAlertCollectionType collection) {
		return Optional.ofNullable(collection).map(WellbeingAlertCollectionType::getWellbeingAlert).map(List::size)
				.orElse(-1);
	}

	@Override
	public List<WellbeingAlertType> getCollectionList(WellbeingAlertCollectionType collection) {
		return Optional.ofNullable(collection).map(WellbeingAlertCollectionType::getWellbeingAlert)
