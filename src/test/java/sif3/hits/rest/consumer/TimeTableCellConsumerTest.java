...
public class TimeTableCellConsumerTest extends BaseTest<TimeTableCellType, TimeTableCellCollectionType> {
    ...
    public void setup() {
        ...
        timeTableCellTester = new ConsumerTest<>(TimeTableCellType.class, "TimeTableCell", TimeTableCellCollectionType.class, "TimeTableCells");
    ...
    public int getCollectionSize(TimeTableCellCollectionType collection) {
        return Optional.ofNullable(collection).map(TimeTableCellCollectionType::getTimeTableCell).map(List::size).orElse(-1);
    }

    @Override
    public List<TimeTableCellType> getCollectionList(TimeTableCellCollectionType collection) {
        return Optional.ofNullable(collection).map(TimeTableCellCollectionType::getTimeTableCell).orElse(new ArrayList<>());
    }
    ...
    public void testUpdateTeacherCover() {
        TimeTableCellType initialObject = getSingleObjectFromXML(timeTableCellTester.getFileContents(testData.getSingleObjectXMLFilename()));
        TimeTableCellType updatedObject = getSingleObjectFromXML(timeTableCellTester.getFileContents(testData.getSingleObjectXMLFilename()));
