...
public class PaymentReceiptConsumerTest extends BaseTest<PaymentReceiptType, PaymentReceiptCollectionType> {
    ...
    public void setup() {
        ...
        paymentReceiptTester = new ConsumerTest<>(PaymentReceiptType.class, "PaymentReceipt", PaymentReceiptCollectionType.class, "PaymentReceipts");
    ...
    public int getCollectionSize(PaymentReceiptCollectionType collection) {
        return Optional.ofNullable(collection).map(PaymentReceiptCollectionType::getPaymentReceipt).map(List::size).orElse(-1);
    }

    @Override
    public List<PaymentReceiptType> getCollectionList(PaymentReceiptCollectionType collection) {
        return Optional.ofNullable(collection).map(PaymentReceiptCollectionType::getPaymentReceipt).orElse(new ArrayList<>());
    }
