...
public class PersonPictureConsumerTest extends BaseTest<PersonPictureType, PersonPictureCollectionType> {
	...
	public void setup() {
		...
		personPictureTester = new ConsumerTest<>(PersonPictureType.class, "PersonPicture",
				PersonPictureCollectionType.class, "PersonPictures");
	...
	@Override
	public void setRefId(PersonPictureType object, String refId) {
		object.setRefId(refId);
	}

	@Override
	public List<PersonPictureType> getCollectionList(PersonPictureCollectionType collection) {
		return Optional.ofNullable(collection).map(PersonPictureCollectionType::getPersonPicture)
				.orElse(new ArrayList<>());
	}

	@Override
	public int getCollectionSize(PersonPictureCollectionType collection) {
		return Optional.ofNullable(collection).map(PersonPictureCollectionType::getPersonPicture).map(List::size)
				.orElse(-1);
	}
