...
public class CollectionStatusConsumerTest extends BaseTest<CollectionStatusType, CollectionStatusCollectionType> {
    ...
    public void setup() {
        ...
        collectionStatusTester = new ConsumerTest<>(CollectionStatusType.class, "CollectionStatus", CollectionStatusCollectionType.class, "CollectionStatuss");
    ...
    public int getCollectionSize(CollectionStatusCollectionType collection) {
        return Optional.ofNullable(collection).map(CollectionStatusCollectionType::getCollectionStatus).map(List::size).orElse(-1);
    }

    @Override
    public List<CollectionStatusType> getCollectionList(CollectionStatusCollectionType collection) {
        return Optional.ofNullable(collection).map(CollectionStatusCollectionType::getCollectionStatus).orElse(new ArrayList<>());
    }
