...
public class InvoiceTestData extends TestData<InvoiceType, InvoiceCollectionType> {
	...
	public static final String ACCOUNT_CODE_1 = "account-code-1";
	public static final String ACCOUNT_CODE_2 = "account-code-2";

	public InvoiceType getTestObject(String refId) {
		IObjectFactory objectFactory = getObjectFactory();
		InvoiceType invoiceType = new InvoiceType();
		invoiceType.setRefId(refId);
		invoiceType.setLocalId(objectFactory.createInvoiceTypeLocalId(LOCAL_ID));
		...
		invoiceType.setChargedLocationInfoRefId(
				objectFactory.createInvoiceTypeChargedLocationInfoRefId(ChargedLocationInfoTestData.REF_ID_1));
		...
		invoiceType.setRelatedPurchaseOrderRefId(
				objectFactory.createInvoiceTypeRelatedPurchaseOrderRefId(PurchaseOrderTestData.REF_ID_1));
		invoiceType.setVoluntary(
				objectFactory.createInvoiceTypeVoluntary(AUCodeSetsYesOrNoCategoryType.fromValue(VOLUNTARY)));
		...
		financialAccountRefIdList.getFinancialAccountRefId()
				.addAll(Arrays.asList(FinancialAccountTestData.REF_ID_1, FinancialAccountTestData.REF_ID_2));
		...
		accountCodeList.getAccountCode().addAll(Arrays.asList(ACCOUNT_CODE_1, ACCOUNT_CODE_2));
		invoiceType.setAccountCodeList(objectFactory.createInvoiceTypeAccountCodeList(accountCodeList));

		if (XML_REF_ID_2.contentEquals(refId)) {
			invoiceType.setAccountCodeList(null);
	...
	public boolean isExampleObject(InvoiceType object, int index) {
		return object != null && object.getFormNumber() != null
				&& FORM_NUMBER_QBE.equals(object.getFormNumber().getValue());
	}
