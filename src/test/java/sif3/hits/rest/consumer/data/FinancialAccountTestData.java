...
public class FinancialAccountTestData extends TestData<FinancialAccountType,FinancialAccountCollectionType> {
    ...
    public FinancialAccountType getTestObject(String refId) {
        ...
        financialAccount.setChargedLocationInfoRefId(objectFactory.createFinancialAccountTypeChargedLocationInfoRefId(ChargedLocationInfoTestData.REF_ID_1));
        financialAccount.setCreationDate(getDate(CREATE_DATE));
        financialAccount.setCreationTime(getDate(CREATE_TIME));
        financialAccount.setDescription(objectFactory.createFinancialAccountTypeDescription(DESCRIPTION + financialAccount.getAccountNumber()));
        ...
        if (REF_ID_5.equals(refId)) {
            ...
            financialAccount.setParentAccountRefId(objectFactory.createFinancialAccountTypeParentAccountRefId(REF_ID_3));
        }
    ...
    public boolean isServicePathObject(FinancialAccountType object, int index) {
        // link to school is via charged location info no way to really check if it's the right school this way.
        return object != null && object.getChargedLocationInfoRefId() != null && object.getChargedLocationInfoRefId().getValue() != null;
    }
