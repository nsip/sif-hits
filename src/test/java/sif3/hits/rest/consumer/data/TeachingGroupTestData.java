...
public class TeachingGroupTestData extends TestData<TeachingGroupType, TeachingGroupCollectionType> {
    ...
    public TeachingGroupType getTestObject(String refId) {
        ...
        teachingGroup.setSchoolInfoRefId(objectFactory.createTeachingGroupTypeSchoolInfoRefId(SchoolInfoTestData.REF_ID_1));
        teachingGroup.setSchoolCourseLocalId(objectFactory.createTeachingGroupTypeSchoolLocalId(SchoolInfoTestData.LOCAL_ID));
        ...
        teachingGroup.setTimeTableSubjectRefId(objectFactory.createTeachingGroupTypeTimeTableSubjectRefId(TimeTableSubjectTestData.REF_ID_1));
        teachingGroup.setTimeTableSubjectLocalId(objectFactory.createTeachingGroupTypeTimeTableSubjectLocalId(TimeTableSubjectTestData.LOCAL_ID));
        ...
        teacher.setStaffPersonalRefId(objectFactory.createTeachingGroupTeacherTypeStaffPersonalRefId(STAFF_MAP.getOrDefault(refId, StaffPersonalTestData.REF_ID_5)));
        teacher.setStaffLocalId(objectFactory.createTeachingGroupTeacherTypeStaffLocalId(StaffPersonalTestData.LOCAL_ID));
    ...
    public StudentListType getFullStudentList() {
        ...
        for (String studentPersonalRefId : StudentPersonalTestData.REF_ID_LIST) {
            ...
            student.setStudentPersonalRefId(objectFactory.createTeachingGroupStudentTypeStudentPersonalRefId(studentPersonalRefId));
            student.setName(objectFactory.createTeachingGroupStudentTypeName(StudentPersonalTestData.getNameOfRecord()));
            student.setStudentLocalId(objectFactory.createTeachingGroupStudentTypeStudentLocalId(StudentPersonalTestData.LOCAL_ID));
    ...
    public List<QueryCriteria> getServicePathQueries() {
        List<QueryCriteria> servicePaths = new ArrayList<>();
        {
            ...
            queryCriteria.addPredicate(new QueryPredicate("StudentPersonals", QueryOperator.EQUAL, StudentPersonalTestData.REF_ID_4));
            servicePaths.add(queryCriteria);
        }
        {
            QueryCriteria queryCriteria = new QueryCriteria();
            queryCriteria.addPredicate(new QueryPredicate("StaffPersonals", QueryOperator.EQUAL, StaffPersonalTestData.REF_ID_4));
            servicePaths.add(queryCriteria);
        }
        return servicePaths;
    }
    ...
    public boolean isServicePathObject(TeachingGroupType object, int index) {
        if (index == 0) {
            return Optional.ofNullable(object).map(TeachingGroupType::getStudentList).map(JAXBElement::getValue).map(StudentListType::getTeachingGroupStudent).orElse(new ArrayList<>()).stream().map(TeachingGroupStudentType::getStudentPersonalRefId).map(JAXBElement::getValue).anyMatch(g -> StudentPersonalTestData.REF_ID_4.equals(g));
        } else if (index == 1) {
            return Optional.ofNullable(object).map(TeachingGroupType::getTeacherList).map(JAXBElement::getValue).map(TeacherListType::getTeachingGroupTeacher).orElse(new ArrayList<>()).stream().map(TeachingGroupTeacherType::getStaffPersonalRefId).map(JAXBElement::getValue).anyMatch(g -> StaffPersonalTestData.REF_ID_4.equals(g));
        }
        return false;
    }
