...
public class PurchaseOrderTestData extends TestData<PurchaseOrderType, PurchaseOrderCollectionType> {
	...
	public PurchaseOrderType getTestObject(String refId) {
		...
		purchaseOrderType.setLocalId(objectFactory.createPurchaseOrderTypeLocalId("local-id"));

		purchaseOrderType.setChargedLocationInfoRefId(
				objectFactory.createPurchaseOrderTypeChargedLocationInfoRefId(ChargedLocationInfoTestData.REF_ID_1));
		...
		purchaseOrderType.setEmployeePersonalRefId(
				objectFactory.createPurchaseOrderTypeEmployeePersonalRefId(StaffPersonalTestData.REF_ID_1));
		purchaseOrderType.setFormNumber(FORM_NUMBER);
		purchaseOrderType.setFullyDelivered(objectFactory
				.createPurchaseOrderTypeFullyDelivered(AUCodeSetsYesOrNoCategoryType.fromValue(FULLY_DELIVERED)));
	...
	private PurchasingItemType getPurchasingItem(int index) {
		...
		expenseAccount.setFinancialAccountRefId(
				objectFactory.createExpenseAccountTypeFinancialAccountRefId(FinancialAccountTestData.REF_ID_1));
		...
		purchasingItem.setItemNumber(
				objectFactory.createPurchasingItemTypeItemNumber(ITEM_NUMBER.multiply(intIndex).toString()));
