...
public class StudentAttendanceTimeListConsumerTest extends BaseTest<StudentAttendanceTimeListType, StudentAttendanceTimeListCollectionType> {
    ...
    public void setup() {
        ...
        studentAttendanceTimeListTester = new ConsumerTest<>(StudentAttendanceTimeListType.class, "StudentAttendanceTimeList", StudentAttendanceTimeListCollectionType.class, "StudentAttendanceTimeLists");
    ...
    public List<StudentAttendanceTimeListType> getCollectionList(StudentAttendanceTimeListCollectionType collection) {
        return Optional.ofNullable(collection).map(StudentAttendanceTimeListCollectionType::getStudentAttendanceTimeList).orElse(new ArrayList<>());
    }

    @Override
    public int getCollectionSize(StudentAttendanceTimeListCollectionType collection) {
        return Optional.ofNullable(collection).map(StudentAttendanceTimeListCollectionType::getStudentAttendanceTimeList).map(List::size).orElse(-1);
    }
    ...
    public void testCreateConstraintError() {
        List<Response> responses = studentAttendanceTimeListTester.testGetSingle(StudentAttendanceTimeListTestData.REF_ID_1);
        ...
        Assert.assertEquals("No Error 500 received", HttpStatus.INTERNAL_SERVER_ERROR.value(), createResponse.getStatus());
        Assert.assertTrue("Message different from expected", createResponse.getError().getMessage().contains("Cannot create object because we are unable to find referenced child objects"));
    }

    @Test
    @Category(IntegrationTest.class)
    public void testUpdateConstraintError() throws Exception {
        StudentAttendanceTimeListType initialObject = getSingleObjectFromXML(studentAttendanceTimeListTester.getFileContents(testData.getSingleObjectXMLFilename()));
        ...
        Assert.assertEquals("No Error 500 received", HttpStatus.INTERNAL_SERVER_ERROR.value(), updateResponse.getStatus());
        Assert.assertTrue("Message different from expected", updateResponse.getError().getMessage().contains("Cannot update object because we are unable to find referenced child objects"));
    }
}
