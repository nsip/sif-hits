...
public class PersonalisedPlanConsumerTest extends BaseTest<PersonalisedPlanType, PersonalisedPlanCollectionType> {
    ...
    public void setup() {
        ...
        personalisedPlanTester = new ConsumerTest<>(PersonalisedPlanType.class, "PersonalisedPlan", PersonalisedPlanCollectionType.class, "PersonalisedPlans");
    ...
    public int getCollectionSize(PersonalisedPlanCollectionType collection) {
        return Optional.ofNullable(collection).map(PersonalisedPlanCollectionType::getPersonalisedPlan).map(List::size).orElse(-1);
    }

    @Override
    public List<PersonalisedPlanType> getCollectionList(PersonalisedPlanCollectionType collection) {
        return Optional.ofNullable(collection).map(PersonalisedPlanCollectionType::getPersonalisedPlan).orElse(new ArrayList<>());
    }
