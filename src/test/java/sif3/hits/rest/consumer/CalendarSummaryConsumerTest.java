...
public class CalendarSummaryConsumerTest extends BaseTest<CalendarSummaryType, CalendarSummaryCollectionType> {
	...
	public void setup() {
		...
		calendarSummaryTester = new ConsumerTest<>(CalendarSummaryType.class, "CalendarSummary",
	...
	@Override
	public void setRefId(CalendarSummaryType object, String refId) {
		object.setRefId(refId);
	}

	@Override
	public List<CalendarSummaryType> getCollectionList(CalendarSummaryCollectionType collection) {
		return Optional.ofNullable(collection).map(CalendarSummaryCollectionType::getCalendarSummary)
				.orElse(new ArrayList<>());
	}

	@Override
	public int getCollectionSize(CalendarSummaryCollectionType collection) {
		return Optional.ofNullable(collection).map(CalendarSummaryCollectionType::getCalendarSummary).map(List::size)
				.orElse(-1);
	}
