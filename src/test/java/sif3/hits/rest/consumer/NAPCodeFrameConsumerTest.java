...
public class NAPCodeFrameConsumerTest extends BaseTest<NAPCodeFrameType, NAPCodeFrameCollectionType> {
    ...
    public void setup() {
        ...
        codeFrameTester = new ConsumerTest<>(NAPCodeFrameType.class, "NAPCodeFrame", NAPCodeFrameCollectionType.class, "NAPCodeFrames");
    }
    ...
    public List<NAPCodeFrameType> getCollectionList(NAPCodeFrameCollectionType collection) {
        return Optional.ofNullable(collection).map(NAPCodeFrameCollectionType::getNAPCodeFrame).orElse(new ArrayList<>());
    }

    @Override
    public int getCollectionSize(NAPCodeFrameCollectionType collection) {
        return Optional.ofNullable(collection).map(NAPCodeFrameCollectionType::getNAPCodeFrame).map(List::size).orElse(-1);
    }
