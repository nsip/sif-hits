...
public class StaffAssignmentConsumerTest extends BaseTest<StaffAssignmentType, StaffAssignmentCollectionType> {
    ...
    public void setup() {
        ...
        staffAssignmentTester = new ConsumerTest<>(StaffAssignmentType.class, "StaffAssignment", StaffAssignmentCollectionType.class, "StaffAssignments");
    ...
    public List<StaffAssignmentType> getCollectionList(StaffAssignmentCollectionType collection) {
        return Optional.ofNullable(collection).map(StaffAssignmentCollectionType::getStaffAssignment).orElse(new ArrayList<>());
    }

    @Override
    public int getCollectionSize(StaffAssignmentCollectionType collection) {
        return Optional.ofNullable(collection).map(StaffAssignmentCollectionType::getStaffAssignment).map(List::size).orElse(-1);
    }
