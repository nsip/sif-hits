...
public class CollectionRoundConsumerTest extends BaseTest<CollectionRoundType, CollectionRoundCollectionType> {
	...
	public void setup() {
		...
		collectionRoundTester = new ConsumerTest<>(CollectionRoundType.class, "CollectionRound",
				CollectionRoundCollectionType.class, "CollectionRounds");
	...
	@Override
	public void setRefId(CollectionRoundType object, String refId) {
		object.setRefId(refId);
	}

	@Override
	public int getCollectionSize(CollectionRoundCollectionType collection) {
		return Optional.ofNullable(collection).map(CollectionRoundCollectionType::getCollectionRound).map(List::size)
				.orElse(-1);
	}

	@Override
	public List<CollectionRoundType> getCollectionList(CollectionRoundCollectionType collection) {
		return Optional.ofNullable(collection).map(CollectionRoundCollectionType::getCollectionRound)
