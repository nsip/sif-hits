...
public class WellbeingAppealConsumerTest extends BaseTest<WellbeingAppealType, WellbeingAppealCollectionType> {
	...
	public void setup() {
		...
		wellbeingAppealTester = new ConsumerTest<>(WellbeingAppealType.class, "WellbeingAppeal",
	...
	@Override
	public void setRefId(WellbeingAppealType object, String refId) {
		object.setRefId(refId);
	}

	@Override
	public int getCollectionSize(WellbeingAppealCollectionType collection) {
		return Optional.ofNullable(collection).map(WellbeingAppealCollectionType::getWellbeingAppeal).map(List::size)
				.orElse(-1);
	}

	@Override
	public List<WellbeingAppealType> getCollectionList(WellbeingAppealCollectionType collection) {
		return Optional.ofNullable(collection).map(WellbeingAppealCollectionType::getWellbeingAppeal)
