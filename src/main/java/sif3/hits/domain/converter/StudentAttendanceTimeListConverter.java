...
@Component
public class StudentAttendanceTimeListConverter
		extends HitsConverter<StudentAttendanceTimeListType, StudentAttendanceTimeList> {
		...
		if (source != null && target != null) {
			...
			if (source.getAttendanceTimes() != null && !source.getAttendanceTimes().isEmpty()) {
				...
				for (StudentAttendanceTime studentAttendanceTime : source.getAttendanceTimes()) {
					...
					attendanceTime.setDurationValue(objectFactory.createAttendanceTimeTypeDurationValue(
							getBigDecimalValue(studentAttendanceTime.getDurationValue())));
					attendanceTime.setAttendanceNote(objectFactory
							.createAttendanceTimeTypeAttendanceNote(studentAttendanceTime.getAttendanceNote()));
					attendanceTime.setAttendanceStatus(getEnumValue(studentAttendanceTime.getAttendanceStatus(),
							AUCodeSetsAttendanceStatusType.class));
					...
					attendanceTime.setTimeTableSubjectRefId(objectFactory.createAttendanceTimeTypeTimeTableSubjectRefId(
							studentAttendanceTime.getTimeTableSubjectRefId()));

					AttendanceCodeType attendanceCodeType = new AttendanceCodeType();
					attendanceCodeType.setCode(getEnumValue(studentAttendanceTime.getAttendanceType(),
							AUCodeSetsAttendanceCodeType.class));

					if (studentAttendanceTime.getOtherCodes() != null
							&& !studentAttendanceTime.getOtherCodes().isEmpty()) {
						...
						attendanceCodeType
								.setOtherCodeList(objectFactory.createAttendanceCodeTypeOtherCodeList(otherCodeList));
					}
					attendanceTime.setAttendanceCode(attendanceCodeType);

					attendanceTimes.getAttendanceTime().add(attendanceTime);
				}
				target.setAttendanceTimes(
						objectFactory.createStudentAttendanceTimeListTypeAttendanceTimes(attendanceTimes));
			}

			if (source.getPeriodAttendanceTimes() != null && !source.getPeriodAttendanceTimes().isEmpty()) {
				List<PeriodAttendanceType> periodAttendances = periodAttendanceConverter
						.toSifModelList(source.getPeriodAttendanceTimes());
				if (periodAttendances != null && !periodAttendances.isEmpty()) {
					...
					target.setPeriodAttendances(
							objectFactory.createStudentAttendanceTimeListTypePeriodAttendances(periodAttendancesType));
				}
			}
		}
	}
	...
	public void toHitsModel(StudentAttendanceTimeListType source, StudentAttendanceTimeList target) {
		if (source != null && target != null) {
			...
			AttendanceTimesType attendanceTimesType = getJAXBValue(source.getAttendanceTimes());
			if (attendanceTimesType != null) {
				for (AttendanceTimeType attendanceTime : attendanceTimesType.getAttendanceTime()) {
					...
					studentAttendanceTime
							.setDurationValue(getBigDecimalValue(getJAXBValue(attendanceTime.getDurationValue())));
					...
					studentAttendanceTime
							.setTimeTableSubjectRefId(getJAXBValue(attendanceTime.getTimeTableSubjectRefId()));
			...
			if (periodAttendances != null && !periodAttendances.getPeriodAttendance().isEmpty()) {
				List<PeriodAttendanceTime> periodAttendanceTimes = periodAttendanceConverter
						.toHitsModelList(periodAttendances.getPeriodAttendance());
