...
public class CalendarDateConverter extends HitsConverter<CalendarDateType, CalendarDate> {
	...
	public void toSifModel(CalendarDate source, CalendarDateType target) {
		if (source != null && target != null) {
			...
			target.setCalendarSummaryRefId(
					objectFactory.createCalendarDateTypeCalendarSummaryRefId(source.getCalendarSummaryRefId()));
			...
			if (StringUtils.isNotBlank(source.getTypeCode()) || !source.getCalendarDateTypeOtherCodes().isEmpty()) {
				...
				List<OtherCode> otherCodes = calendarDateTypeOtherCodeConverter
						.toSifModelList(source.getCalendarDateTypeOtherCodes());
				otherCodeListType.getOtherCode().addAll(otherCodes);
				if (otherCodes != null && !otherCodes.isEmpty()) {
					calendarDateType
							.setOtherCodeList(objectFactory.createCalendarDateInfoTypeOtherCodeList(otherCodeListType));
				}

				target.setCalendarDateType(calendarDateType);
			}
			target.setCalendarDateNumber(
					objectFactory.createCalendarDateTypeCalendarDateNumber(getLongValue(source.getNumber())));

			if (source.getStudentAttendanceAttendanceValue() != null
					&& source.getStudentAttendanceCountsTowardsAttendance() != null) {
		...
		if (source != null && target != null) {
			...
			if (calendarDateType != null) {
				...
				if (otherCodeList != null) {
					List<CalendarDateTypeOtherCode> otherCodes = calendarDateTypeOtherCodeConverter
			...
			if (studentAttendance != null) {
				...
				target.setStudentAttendanceAttendanceValue(null);
				target.setStudentAttendanceCountsTowardsAttendance(null);
			}
		}
	}
}
