...
import sif3.hits.domain.model.AddressCollectionReportingStudentAddress;
import sif3.hits.domain.model.AddressCollectionReportingStudentAddressStatisticalArea;

@Component
public class AddressCollectionReportingStudentAddressConverter extends BaseAddressConverter<AddressCollectionReportingStudentAddress> {

    @Autowired
    AddressCollectionReportingStudentAddressStatisticalAreaConverter statisticalAreaConverter;

    public AddressCollectionReportingStudentAddressConverter() {
        super(AddressCollectionReportingStudentAddress.class);
    }

    @Override
    public void toSifModel(AddressCollectionReportingStudentAddress source, AddressType target) {
        if (source != null && target != null) {
            ...
            if (source.getStatisticalAreas() != null) {
                ...
                for (AddressCollectionReportingStudentAddressStatisticalArea statisticalArea : source.getStatisticalAreas()) {
    ...
    @Override
    public void toHitsModel(AddressType source, AddressCollectionReportingStudentAddress target) {
        if (source != null && target != null) {
            ...
            if (statisticalAreasType != null) {
                for (StatisticalAreaType statisticalAreaType : statisticalAreasType.getStatisticalArea()) {
                	AddressCollectionReportingStudentAddressStatisticalArea statisticalArea = statisticalAreaConverter.toHitsModel(statisticalAreaType);
