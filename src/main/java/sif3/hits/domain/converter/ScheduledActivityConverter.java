...
public class ScheduledActivityConverter extends HitsConverter<ScheduledActivityType, ScheduledActivity> {
	...
	public void toSifModel(ScheduledActivity source, ScheduledActivityType target) {
		if (source != null && target != null) {
			...
			target.setTimeTableCellRefId(
					objectFactory.createScheduledActivityTypeTimeTableCellRefId(source.getTimeTableCellRefId()));
			target.setTimeTableRefId(
					objectFactory.createScheduledActivityTypeTimeTableRefId(source.getTimeTableRefId()));
			target.setTimeTableSubjectRefId(
					objectFactory.createScheduledActivityTypeTimeTableSubjectRefId(source.getTimeTableSubjectRefId()));
			...
			target.setActivityType(objectFactory.createScheduledActivityTypeActivityType(
					getEnumValue(source.getType(), AUCodeSetsScheduledActivityTypeType.class)));
			...
			if (StringUtils.isNotBlank(source.getYearLevels())) {
			...
			if (StringUtils.isNotBlank(source.getOverride()) || StringUtils.isNotBlank(source.getDateOfOverride())) {
	...
	public void toHitsModel(ScheduledActivityType source, ScheduledActivity target) {
		if (source != null && target != null) {
			...
			if (override != null) {
				...
				target.setOverride(null);
				target.setDateOfOverride(null);
			}
		}
	}
}
