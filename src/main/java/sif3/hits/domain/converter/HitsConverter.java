...
public abstract class HitsConverter<S, H> implements UsesConstants {
	...
	protected SoftwareVendorInfoContainerType getSoftwareVendorInfo(
	...
	protected SoftwareVendorInfoContainerType getSoftwareVendorInfo(
			JAXBElement<SoftwareVendorInfoContainerType> softwareVendorInfo) {
		return getSoftwareVendorInfo(getJAXBValue(softwareVendorInfo));
	}

	protected YearLevelType getYearLevelType(String yearLevelCode) {
		YearLevelType result = null;
		if (StringUtils.isNotBlank(yearLevelCode)) {
			result = getObjectFactory().createYearLevelType();
			result.setCode(getEnumValue(yearLevelCode, AUCodeSetsYearLevelCodeType.class));
		}
		return result;
	}

	protected String getYearLevelType(YearLevelType yearLevelType) {
		String result = null;
		if (yearLevelType != null) {
			result = getEnumValue(yearLevelType.getCode());
		}
		return result;
	}
