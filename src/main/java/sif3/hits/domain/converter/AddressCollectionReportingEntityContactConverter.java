...
@Component
public class AddressCollectionReportingEntityContactConverter extends BaseEntityContactConverter<AddressCollectionReportingEntityContact> {
    ...
    public void toHitsModel(EntityContactInfoType source, AddressCollectionReportingEntityContact target) {
        if (source != null && target != null) {
            ...
            target.setName(name);
            AddressCollectionReportingEntityContactAddress address = addressConverter.toHitsModel(getJAXBValue(source.getAddress()));
            if (address != null) {
