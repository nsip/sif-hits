...
public class NAPStudentResponseSetConverter extends HitsConverter<NAPStudentResponseSetType, NAPStudentResponseSet> {
	...
	public void toSifModel(NAPStudentResponseSet source, NAPStudentResponseSetType target) {
		if (source != null && target != null) {
			...
			target.setCalibrationSampleFlag(objectFactory
					.createNAPStudentResponseSetTypeCalibrationSampleFlag(source.getCalibrationSampleFlag()));
			target.setEquatingSampleFlag(
					objectFactory.createNAPStudentResponseSetTypeEquatingSampleFlag(source.getEquatingSampleFlag()));
			target.setPathTakenForDomain(
					objectFactory.createNAPStudentResponseSetTypePathTakenForDomain(source.getPathTakenForDomain()));
			...
			if (source.getDomainScores() != null && !source.getDomainScores().isEmpty()) {
				target.setDomainScore(objectFactory.createNAPStudentResponseSetTypeDomainScore(
						domainScoreConverter.toSifModel(source.getDomainScores().get(0))));
			}
			...
			if (studentPersonal != null) {
				...
				target.setStudentPersonalRefId(
						objectFactory.createNAPStudentResponseSetTypeStudentPersonalRefId(studentPersonal.getRefId()));
			}

			// NAP Testlet List
			List<NAPTestletResponseType> testletList = studentResponseTestletConverter
					.toSifModelList(source.getTestletList());
			if (testletList != null && !testletList.isEmpty()) {
				NAPStudentResponseTestletListType napTestletListType = objectFactory
						.createNAPStudentResponseTestletListType();
	...
	public void toHitsModel(NAPStudentResponseSetType source, NAPStudentResponseSet target) {
		if (source != null && target != null) {
			...
			NAPTest napTest = null;
			if (testRefId != null) {
				napTest = napTestDAO.findOne(testRefId);
			}
			if (napTest == null && testLocalId != null) {
			...
			if (studentPersonal == null) {
				List<StudentPersonal> studentPersonals = studentPersonalDAO
						.findWithOtherId(PersonalStudentIdService.PERSONAL_STUDENT_IDENTIFIER_TYPE, studentLocalId);
			...
			if (napTestletListType != null) {
				target.getTestletList()
						.addAll(studentResponseTestletConverter.toHitsModelList(napTestletListType.getTestlet()));
