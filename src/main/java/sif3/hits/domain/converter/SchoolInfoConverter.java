...
public class SchoolInfoConverter extends HitsConverter<SchoolInfoType, SchoolInfo> implements UsesConstants {
	...
	public void toSifModel(SchoolInfo source, SchoolInfoType target) {
		if (source != null && target != null) {
			...
			AUCodeSetsSchoolLevelType schoolType = getEnumValue(source.getSchoolType(),
					AUCodeSetsSchoolLevelType.class);
			...
			target.setSchoolGeographicLocation(objFactory.createSchoolInfoTypeSchoolGeographicLocation(
					getEnumValue(source.getAddressGeographicLocation(), AUCodeSetsSchoolLocationType.class)));
			...
			AUCodeSetsSchoolLevelType campusType = getEnumValue(source.getCampusType(),
					AUCodeSetsSchoolLevelType.class);
			campus.setCampusType(objFactory.createCampusContainerTypeCampusType(campusType));
			campus.setParentSchoolId(
					objFactory.createCampusContainerTypeParentSchoolId(source.getCampusParentSchoolId()));
	...
	public void toHitsModel(SchoolInfoType source, SchoolInfo target) {
		if (source != null && target != null) {
			...
			if (addressList != null && addressList.getAddress() != null && addressList.getAddress().size() > 0) {
				...
				target.setAddressStateProvince(null);
				target.setAddressCity(null);
				target.setAddressPostalCode(null);
				target.setAddressStreetNumber(null);
				target.setAddressStreetName(null);
				target.setAddressLatitude(null);
				target.setAddressLongitude(null);
			}
