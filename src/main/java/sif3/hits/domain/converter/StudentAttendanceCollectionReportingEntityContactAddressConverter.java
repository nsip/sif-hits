...
@Component
public class StudentAttendanceCollectionReportingEntityContactAddressConverter
		extends BaseAddressConverter<StudentAttendanceCollectionReportingEntityContactAddress> {

	@Autowired
	StudentAttendanceCollectionReportingEntityContactAddressStatisticalAreaConverter statisticalAreaConverter;

	public StudentAttendanceCollectionReportingEntityContactAddressConverter() {
		super(StudentAttendanceCollectionReportingEntityContactAddress.class);
	}

	@Override
	public void toSifModel(StudentAttendanceCollectionReportingEntityContactAddress source, AddressType target) {
		if (source != null && target != null) {
			...
			if (source.getStatisticalAreas() != null) {
				...
				for (StudentAttendanceCollectionReportingEntityContactAddressStatisticalArea statisticalArea : source
						.getStatisticalAreas()) {
	...
	@Override
	public void toHitsModel(AddressType source, StudentAttendanceCollectionReportingEntityContactAddress target) {
		if (source != null && target != null) {
			...
			if (statisticalAreasType != null) {
				for (StatisticalAreaType statisticalAreaType : statisticalAreasType.getStatisticalArea()) {
					StudentAttendanceCollectionReportingEntityContactAddressStatisticalArea statisticalArea = statisticalAreaConverter
