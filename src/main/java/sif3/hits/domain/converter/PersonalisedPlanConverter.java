...
@Component
public class PersonalisedPlanConverter extends HitsConverter<PersonalisedPlanType, PersonalisedPlan> implements UsesConstants {
    ...
    public void toSifModel(PersonalisedPlan source, PersonalisedPlanType target) {
        if (source != null && target != null) {
            ...
            target.setPersonalisedPlanCategory(getEnumValue(source.getPersonalisedPlanCategory(), AUCodeSetsPersonalisedPlanType.class));
            target.setPersonalisedPlanStartDate(getDateValue(source.getPersonalisedPlanStartDate()));
            target.setPersonalisedPlanEndDate(objectFactory.createPersonalisedPlanTypePersonalisedPlanEndDate(getDateValue(source.getPersonalisedPlanEndDate())));
            target.setPersonalisedPlanReviewDate(objectFactory.createPersonalisedPlanTypePersonalisedPlanReviewDate(getDateValue(source.getPersonalisedPlanReviewDate())));
            target.setPersonalisedPlanNotes(objectFactory.createPersonalisedPlanTypePersonalisedPlanNotes(source.getPersonalisedPlanNotes()));
            target.setAssociatedAttachment(objectFactory.createPersonalisedPlanTypeAssociatedAttachment(source.getAssociatedAttachment()));
    ...
    public void toHitsModel(PersonalisedPlanType source, PersonalisedPlan target) {
        if (source != null && target != null) {
            ...
            if (documentList != null && !documentList.getDocument().isEmpty()) {
                List<PersonalisedPlanDocument> wellbeingDocuments = documentConverter.toHitsModelList(documentList.getDocument());
                if (wellbeingDocuments != null && !wellbeingDocuments.isEmpty()) {
